...
public class CampusTaskPlanner {
    public CampusAgentPlan createPlan(CampusEventGoal goal) {
        ...
        List<AgentTask> tasks = List.of(
                ...
                task("estimate_supplies", "生成具体物资数量清单", TaskCapability.TOOL,
                        List.of("resolve_constraints", "research_weather", "design_agenda"),
                        "物资名称、数量、单位、单价控制上限、依据和核验状态"),
                task("allocate_budget", "计算并分配预算", TaskCapability.TOOL,
                        List.of("match_venue", "design_agenda", "plan_staffing", "estimate_supplies"),
                        "逐项数量、单价控制上限、小计、待询价额度、人均成本和备用金"),
