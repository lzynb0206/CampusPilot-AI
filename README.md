...
一个基于 Spring Boot 的自主规划型校园活动策划 Agent。用户只需输入一句需求，CampusPilot 就会先生成可直接修改的校园通用方案；日期、人数、预算或具体教室没有提供时不会阻断回答，可在后续再补充精细化。系统内部会完成校园规则、天气、场地、流程、人员、物资、预算和风险检查，用户端只展示活动方案，不展示 Agent 任务状态。
...
| 完整策划书输出 | 面向用户的 10 个方案章节，不暴露内部任务记录 | 已完成 |
...
| `CAMPUS_CONVERSATION_TTL_MINUTES` | `120` | 每位微信联系人的校园活动上下文保留分钟数 |
...
- 内部仍保留 16 位运行编号和续跑能力，但用户版方案不展示任务编号、完成状态或恢复记录。
- 完整策划书生成成功后自动删除检查点，避免长期使用过期天气或旧资料。
- 检查点目录已加入 `.gitignore`，不会提交到仓库。

### 多轮活动上下文

校园活动首次生成后，系统会按微信联系人分别保存当前活动的名称、日期、城市、学校、场地、开始时间、人数和预算。用户后续只需发送“换到西区食堂三楼”“人数改80”“下周三，15点30开始”等补充，系统会合并到原方案并重新生成；继续问“这个活动还有什么风险”时，普通问答也会获得当前活动摘要。不同联系人之间不会串方案。上下文默认保留 120 分钟；发送“重新策划”“清除方案”或“结束策划”可以立即重置。该上下文保存在当前进程内，应用重启后需要重新发送活动需求。

### 防止编造的边界

- 缺少活动名称、日期、城市、人数或预算时，先按通用假设生成方案：默认 50 人、2000 元，日期和校区留待补充。日期或城市缺失时不调用真实天气工具，而是给出通用备用方案。
- 9 月 20 日等远期日期超出天气预报范围时返回 `TOO_EARLY`，只给复查日期和备用方案，不把当前实况冒充未来预报。
- 心知天气账号返回范围不足时使用 `RECHECK_REQUIRED`；接口失败时使用 `QUERY_FAILED`。
- 本校制度资料缺失时，先展示大多数学校通用的审批、安全、预算和隐私规则；具体审批部门、申请时限和报销细则作为可选补充，不阻断方案生成。
- 场地先给出“教学楼”“操场或体育场”“礼堂或活动中心”等区域级建议，具体楼号、教室号或场地名称可后续补充；宣传材料只生成草稿，不自动发布。
- 单独补充“在某大学某楼举行”时会合并联系人已有方案并重新生成；只有明确出现“天气”“气温”“下雨”等天气信号才进入天气查询，城市名本身不会触发天气回复。
...
- CampusPilot 完整目标生成 10 章节用户版活动方案
- CampusPilot 缺少约束时仍生成通用方案，日期或城市缺失时不调用天气工具
- 远期天气不生成虚构预报，并包含复查节点和室内备用方案
- 校园规则先显示通用做法，本校细则只作后续补充
...
- 校园活动上下文按微信联系人隔离，可连续更新学校、场地、人数、预算、日期和开始时间
- 校园活动上下文支持手动清除和超时过期，过期后不会错误套用旧方案
- HttpClient5 连接池容量和 5 秒/30 秒超时配置
- 超大工具结果进入下一轮前会被截断
- 默认本地意图分类在没有 API Key 时也能识别聊天、天气和生图
- 校园活动地点补充不会因包含城市名而误触发天气查询
